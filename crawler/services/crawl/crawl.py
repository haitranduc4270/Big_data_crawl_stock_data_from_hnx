...
def process_ssi_data (spark, data, work):
    kafka_producer_instance = kafka_producer.start_kafka_producer(kafka_bootstrap_servers)

    df = spark.sparkContext.parallelize(data['data']).map(lambda x: json.dumps(x))
    df = spark.read.json(df)

    data_dir = hadoop_namenode + work['hadoop_dir'] + data['timestamp'].strftime(time_format) + '.json'
    ...
    print('Success get data from ' + work['data'] + ' at ' +  data['timestamp'].strftime(time_format))
...
def start_crawl (spark, work):
    if work['data'] == ssi_stock_data_api:
        data = get_stock_real_times_by_group(work['source']['url'], work['source']['body'])
        if (data):
